...
public class Pop3 implements IAccesType {
	...
	public IEmail read(IClient client, IHeader header)
			throws CannotFindEmailException, NoLoggedUserException, exception.CannotFindEmailException, CannotFindUserException {
	...
	public ArrayList<IEmail> changeToClientEmails(List<ServerEmail> server_emails) {
		...
		for (ServerEmail server_email : server_emails) {
			...
			IHeader client_head = this.changeToClientHead(server_email
					.getHead());
			ArrayList<IAttachment> client_attachment = this
					.changeToClientAttachment(server_email.getAttachment());

			IEmail email = new Email(client_head, server_email.getBody(),
					client_attachment, true);
			client_emails.add(email);
		}
		return client_emails;
	}

	private ArrayList<IAttachment> changeToClientAttachment(
			List<ServerEmailAttachment> server_attachs) {
		...
		for (ServerEmailAttachment server_attach : server_attachs) {
			IAttachment attachment = new Attachment(
					server_attach.getFileName(), server_attach.getFile());
	...
	private IHeader changeToClientHead(ServerEmailHead head) {
		...
		return new Header(head.getSender(), head.getReciver(), subject,
				head.getDate());
	}
